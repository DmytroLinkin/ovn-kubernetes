...
func newSharedGateway(nc *DefaultNodeNetworkController, subnets []*net.IPNet,
	...
	gw.initFunc = func() error {
		...
		if config.OvnKubeNode.Mode == types.NodeModeFull {
			if err := setNodeMasqueradeIPOnExtBridge(gw.gwIface); err != nil {
				return fmt.Errorf("failed to set the node masquerade IP on the ext bridge %s: %v", gw.gwIface, err)
			}

			if err := addMasqueradeRoute(gw.gwIface, nc.name, gwBridge.ips, nc.watchFactory); err != nil {
		...
		if err := addHostMACBindings(gw.gwIface); err != nil {
