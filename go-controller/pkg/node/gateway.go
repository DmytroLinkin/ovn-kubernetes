...
func gatewayInitInternal(nc *DefaultNodeNetworkController, nodeAnnotator kube.Annotator) (*gateway,
	*bridgeConfiguration, *bridgeConfiguration, error) {
	...
	gw := &gateway{
		...
		gwIface:      gatewayBridge.bridgeName,
	}

	if gatewayBridge.bypassRep != "" {
		// In bypass configuration bridge is not a gateway interface
		gw.gwIface = gwIface
	}
...
func bridgeForRegularInterface(iface string) (*bridgeConfiguration, string, error) {
...
func bridgeForInterface(intfName, nodeName, physicalNetworkName string, kubeNodeIP net.IP) (*bridgeConfiguration, error) {
	...
	if err == nil {
		...
		if res, gwIntf, err = bridgeForRegularInterface(intfName); err != nil {
			return nil, err
		}
	}
